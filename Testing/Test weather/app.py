...
class NOAAWeatherApp:
    ...
    def get_point_data(self, latitude, longitude):
        ...
        try:
            print(f"Fetching point {url}")
            response = requests.get(url, headers=self.headers, timeout=10)
            print(f"Point data status: {response.status_code}")
            if response.status_code == 200:
                return response.json()
            else:
                print(f"Point data error: {response.text}")
    ...
    def get_hourly_forecast(self, latitude, longitude):
        ...
        if point_data:
            try:
                ...
                print(f"Fetching hourly: {forecast_url}")
                response = requests.get(forecast_url, headers=self.headers, timeout=10)
                print(f"Hourly status: {response.status_code}")
                
                if response.status_code == 200:
                    return response.json()
                else:
                    print(f"Hourly error: {response.text}")
            except Exception as e:
    ...
    def calculate_advanced_prediction(self, city_name):
        ...
        """
        print(f"\n=== Starting prediction for {city_name} ===")
        ...
        try:
            ...
            if forecast and 'properties' in forecast:
                ...
                for period in periods[:3]:  # Look at first 3 periods
                    if 'today' in period['name'].lower() or 'this afternoon' in period['name'].lower():
                        ...
                        print(f"Added forecast temp: {period['temperature']}°F from {period['name']}")
        except Exception as e:
            print(f"Error processing forecast: {e}")
        
        # 2. Get hourly forecast for more granular data
        hourly_temps = []
        hourly_data = []  # Store both temp and time
        try:
            ...
            if hourly and 'properties' in hourly:
                ...
                if hourly_temps:
                    ...
                    print(f"Added hourly max: {hourly_max}°F from {len(hourly_temps)} hours")
        except Exception as e:
            print(f"Error processing hourly: {e}")
        
        # 3. Get current observation as baseline
        current_temp = None
        try:
            ...
            if stations_data and stations_data.get('features'):
                ...
                if obs and 'properties' in obs:
                    ...
                    if temp and temp.get('value') is not None:
                        ...
                        # Don't add current temp to predictions, just store it
        ...
        if not temp_predictions:
            print("No temperature predictions available")
            return None
        
        print(f"Total predictions: {temp_predictions}")
        
        # Use weighted average with more weight on recent/accurate sources
        ...
        # Determine prediction range (tighter range for higher confidence)
        if confidence >= 90:
            range_offset = 1
        elif confidence >= 75:
            range_offset = 2
        ...
        print(f"Prediction complete: {predicted_high}°F (confidence: {confidence}%)")
        return result
...
def compare_predictions():
    ...
    try:
        ...
        print(f"ERROR in compare_predictions: {e}")
        traceback.print_exc()
        return jsonify({'error': str(e)}), 500
